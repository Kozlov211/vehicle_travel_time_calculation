...
template <>
inline void AssertEqual<double, double>(const double& t, const double& u, const std::string& hint) {
    const double EPSILON = 0.01;
    if ((std::abs(t - u) > EPSILON)) {
        std::ostringstream os;
        os << "Assertion failed: " << t << " != " << u;
        if (!hint.empty()) {
            os << " hint: " << hint;
...
/**
 * Сравнивает значения контейнеров t и u.
 * Строка hint содержит подсказку, которая выводится, если тест провален.
 *
 * Пример:
 *  void Test() {
...
void AssertEqualContainer(const Container& t, const Container& u, const std::string& hint = {}) {
    AssertEqual(t.size(), u.size(), "size don't equal");
    for (size_t i = 0; i < t.size(); ++i) {
        AssertEqual(t[i], u[i], hint);
    }
}

template <>
inline void AssertEqualContainer<std::vector<std::vector<double>>>(const std::vector<std::vector<double>>& t, const std::vector<std::vector<double>>& u, const std::string& hint) {
    AssertEqual(t.size(), u.size(), "size don't equal");
    for (size_t i = 0; i < t.size(); ++i) {
        AssertEqualContainer(t[i], u[i], hint);
    }
}

template <class Type>
void AssertEqualRange(const Type* begin1, const Type* end1, const Type* begin2, const Type* end2, const std::string& hint = {}) {
    size_t size1 = std::distance(begin1, end1);
    size_t size2 = std::distance(begin2, end2);
    AssertEqual(size1, size2, "size don't equal");
    for (size_t i = 0; i < size1; ++i) {
        AssertEqual(begin1[i], begin2[i], hint);
    }
}
